                    ...
                    char *path)
{
    ...
    for (p=l_name,q=(char *)name;(*p=(char)tolower(*q));p++,q++) ;					//�ַ���->Сд��ĸ
    for (p=u_name,q=(char *)name;(*p=(char)toupper(*q));p++,q++) ;					//�ַ���->��д��ĸ
    
    for (p=buff,q=(char *)file;(*p=*q);p++,q++) {									//buffȡֵ
...
/* parse field strings separated by spaces �������ɿո�ָ����ַ�����-----------------------------------*/
...
static void remot2local(const char *remot, const char *dir, char *local)
{
    ...
    if ((p=strrchr(remot,'/'))) p++; else p=(char *)remot;					//strrchr������ָ���ַ���һ��λ��
...
static int add_path(paths_t *paths, const char *remot, const char *dir)
{
    ...
    if (!(paths->path[paths->n].remot=(char *)malloc(1023/*strlen(remot)*/+1))||
        !(paths->path[paths->n].local=(char *)malloc(1023/*strlen(local)*/+1))) {				//�����ϵ����
        free_path(paths);
        return 0;
    }
                    ...
                    paths_t *paths)
{
    ...
    if (strstr(url->path,"%N")) {							//URL��%N	
        for (i=seqnos;i<=seqnoe;i++) {
            genpath(url->path,sta,time,i,remot);			//���ݿռ���������ں����ͷ�ʱ��������
        ...
        if (time_p.time) {
            ...
            if (!strcmp(remot_p,remot)) return 1;			//������һ�ĵ�һ�£�����ִ�У�
        }
        if (!add_path(paths,remot,dir_t)) return 0;
    }
    return 1;
                     ...
                     paths_t *paths)
{
    ...
    if (strstr(url->path,"%s")||strstr(url->path,"%S")) {						//URL��%s ��վ
        for (i=0;i<nsta;i++) {
            if (!gen_path(time,time_p,seqnos,seqnoe,url,stas[i],dir,paths)) {	//����������Ǵ���һ������
                return 0;
            }
        }
    }
                    ...
                    const char *proxy)
{
    ...
    if (!(fp=fopen(FTP_LISTING,"r"))) return 0;			//����Ƿ���Դ�listening	
    fclose(fp);
    return 1;
}
...
/* test file in remote file list ---------------------------------------------*/
static int test_list_2021(const path_t *path)
{
	FILE *fp;
	char buff[1024], *file, *list, *p;
	int i;
                     ...
                     FILE *fp)														
{
    ...
    if (proto==0&&!test_list_2021(path)) {							
        showmsg("STAT=x");
    ...
    if (!(opts&DLOPT_KEEPCMP)&&(p=strrchr(path->local,'.'))&&
        (!strcmp(p,".z")||!strcmp(p,".gz")||!strcmp(p,".zip")||
         !strcmp(p,".Z")||!strcmp(p,".GZ")||!strcmp(p,".ZIP"))) {
        if (rtk_uncompress(path->local,tmpfile)) {			//-1:Ϊ��
            remove(path->local);			
        }
        else {
...
*          int    seqnos    I   sequence number start ���кſ�ʼ
...
*          char   *remote_p I   previous remote file path ��ǰԶ���ļ�·��
*          char   *usr      I   login user for ftp
*          char   *pwd      I   login password for ftp
*          char   *proxy    I   proxy server address ������������ַ
                   ...
                   const char *proxy, int opts, char *msg, FILE *fp)						
{
    ...
    while (timediff(ts,te)<1E-3) {		
        for (i=0;i<nurl;i++) {	
            if (!gen_paths(ts,ts_p,seqnos,seqnoe,urls+i,stas,nsta,dir,&paths)) {	//path������·��������			
                free_path(&paths);
                return 0;
            }
        }
    ...
    for (i=0;i<paths.n;i++) {	
		...
		sprintf(str, "%s->%s (%d/%d)\n", paths.path[i].remot, paths.path[i].local, i + 1, paths.n);
