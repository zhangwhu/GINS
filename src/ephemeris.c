	...
	double *var)
{
	...
	*var = var_uraeph(sys, eph->sva);				//利用sva实现方差设定
}
...
*          signal transmission time卫星发射信号时刻
*-----------------------------------------------------------------------------*/
	...
	int ephopt, double *rs, double *dts, double *var, int *svh)
{
	...
	for (i = 0; i<n&&i<2 * MAXOBS; i++) {
		trace(2, "%s sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n",
