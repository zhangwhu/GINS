...
static int rescode(int iter, const obsd_t *obs, int n,  
	double *rs, const double *dts, const double *vare, const int *svh,
	const nav_t *nav, const double *x, const prcopt_t *opt,
	double *v, double *H, double *var, double *azel, int *vsat,
	double *resp, int *ns, int *exc)
{
...
/* validate solution ---------------------------------------------------------*/
static int valsol(const double *azel, const int *vsat, int n, const prcopt_t *opt, 
	const double *v, int nv, int nx, char *msg, int *nsat, double *pdop)
{
...
static int estpos(const obsd_t *obs, int n, double *rs, const double *dts,
	...
	sol_t *sol, double *azel, int *vsat, double *resp, char *msg)
{
	...
	v = mat(n + 4, 1); H = mat(NX, n + 4); var = mat(n + 4, 1);  exc = imat(n, 1);

	for (i = 0; i < 3; i++) x[i] = sol->rr[i];
	
	for (i = 0; i < n; i++) exc[i] = 0;
	...
	sol_t *sol, double *azel, ssat_t *ssat,char *msg)			
{
	...
	if (!ssat)	{ strcpy(msg, "spp ssat is null");    return 0; }			
	...
	if (ssat) {												
		...
		for (i = 0; i<n; i++) {
			...
			if (!vsat[i] || fabs(resp[i]) > opt_.threscheck[4]) continue;
