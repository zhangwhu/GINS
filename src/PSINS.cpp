...
CVect3 sv2att(const CVect3 &fb, double yaw0=0.0, const CVect3 &fn=CVect3(0,0,1))
{
    CVect3 phi = fb*fn;
...
CQuat a2qua(double pitch, double roll, double yaw)				// 东北天312转换
{
...
int CKalman::RAdaptive(int i, double r, double Pr)		// 以系统为参考
{
	double rr=r*r-Pr;
...
{
	Zfd.dd[i] = Zfd.dd[i]*(1-zfdafa) + Zk.dd[i]*zfdafa;	// 以量测为参考
...
void fusion(double *x1, double *p1, const double *x2, const double *p2, int n=9, double *xf=NULL, double *pf=NULL)
{
	...
	if(n<100) {  // n<100 for att(1:3), n>100 for no_att(1:3)
		x10 = x1, xf0 = xf;
		att1 = *(CVect3*)x1; 
		*(CVect3*)x2 = qq2phi(a2qua(*(CVect3*)x2),a2qua(att1));		//Cnb Cn'b
		*(CVect3*)x1 = O31;
	}
	int j;
...
double Huber(double dv, float c) 
{
	if (dv < c) return 1;
	else return dv / c;
}
...
//				if((iter-2*Ft.row)%2==0)
				if(iter%2==0)
				{
					...
					// double dv  = fabs(innovation)/sqrt(Pz0); 	// By zh in 23-08-08
					// double alpha = Huber(dv, 2.0);
					// if (dv > 1) alpha = 5;
					// Rt(row) = alpha * Rt(row); 
...
{
	posGNSSdelay = vnGNSSdelay = yawGNSSdelay = dtGNSSdelay = dyawGNSS = -0.0;	//dyawGNSS:安装角误差
...
void CSINSGNSS::SetFt(int nnq)		
{
	...
	Ft.SetMat3(3,0,sins.Mva), Ft.SetMat3(3,3,sins.Mvv), Ft.SetMat3(3,6,sins.Mvp), Ft.SetMat3(3,12,sins.Cnb);   //看下效果
	...
	if(nnq==20) {
		...
		Ft.SetMat3(0,19, Cwz);				// 19-21 dKGz		//估计z轴三个
	}
	else if(nnq>=28) {
...
void CSINSGNSS::SetHk(int nnq)
{
	if(nnq>=18) {     // GNSS lever				
		CMat3 CW=sins.Cnb*askew(sins.webbar), MC=sins.Mpv*sins.Cnb;			// 除SINS外的状态
		Hk.SetMat3(0,15, -CW);
		Hk.SetMat3(3,15, -MC);						
	}
	if(nnq>=19) {    // GNSS dt
...
void CSINSGNSS::Feedback(int nnq, double fbts)
{
	...
	if(nnq==20) {
		...
		sins.Kg.e20*=IdKGzz, sins.Kg.e21*=IdKGzz, sins.Kg.e22*=IdKGzz;  // 19 dKGzz  参考4.2.1惯性传感器测量误差模型
	}
	else if(nnq==22) {
		CMat3 IdKGz(1.0,0.0,-FBXk.dd[19], 0.0,1.0,-FBXk.dd[20], 0.0,0.0,1.0-FBXk.dd[21]);	// 正常的处理逻辑单位阵操作
		sins.Kg = IdKGz*sins.Kg;			// 19-21 dKGz	3个和z轴相关的状态
	}
	else if(nnq>=28) {
		CMat3 IdKG = I33-(~(*(CMat3*)&FBXk.dd[19]));
		sins.Kg = IdKG*sins.Kg;				// 19-27 dKG	9个状态全部一起估计
	}
	if(nnq>=34) {
		CMat3 IdKA(1.0-FBXk.dd[28],            0.0,           0.0,
			          -FBXk.dd[29],1.0-FBXk.dd[31],           0.0,
					  -FBXk.dd[30],   -FBXk.dd[32],1.0-FBXk.dd[33]);
		sins.Ka = IdKA*sins.Ka;				// 28-33 dKA    6维加速度状态
	}
}
...
void CSINSGNSS::SetMeasGNSS(const CVect3 &posgnss, const CVect3 &vngnss, double yawgnss, double qfactor)
{
	...
	{
		Zk.dd[yawHkRow] = -diffYaw(avpi.att.k, yawgnss+dyawGNSS);				// 注意：姿态失准角和方位失准角间的关系
...
void CSINSGNSSDR::SetMeas(void)
{
	if(*gnsslost>5.0)
	{
		*(CVect3*)&Zk.dd[6] = sins.posL - posDRL;  // SINS/DR			由于是两个系统 所以才让其发挥作用
		SetMeasFlag(000700);
	}
}
...
int CSINSGNSSOD::ODVelUpdate(double dS)
{
	...
	dS *= Kod;
	distance += dS;
	CVect3 dSn = sins.Cnb*(CVect3(Cbo.e01*dS,Cbo.e11*dS,Cbo.e21*dS)-sins.imu.phim*lvOD);  // dSn = Cnb*Cbo*dSb（转向也可以使用）
	odmeast += sins.nts;
...
{
	int res = CSINSGNSS::Update(pwm, pvm, nn, ts, nSteps);
	sins.lever(lvOD, &posOD, &vnOD);														//imu、GNSS、OD位置分离
	ODVelUpdate(dS);
	return res;
}
...
void CAutoDrive::ZIHRtest(void)								// 参考：https://zhuanlan.zhihu.com/p/115529319
{
	wzhd.Update(sins.imu.phim.k/sins.nts);
	if(wzhd.retres==3 && sins.fn.k>9.5) {
		Zk.dd[16] = wzhd.meanwpre-sins.eth.wnie.k;			// 测量信息：陀螺零偏（初始化完成后的操作）
		SetMeasFlag(0200000);
...
int CAutoDrive::Update(const CVect3 *pwm, const CVect3 *pvm, double dS, int nn, double ts, int nSteps)
{
	...
	if(odVelOK) {
		Hk.SetMat3(6, 0, odMphi); Hk.SetMat3(6, 3, odMvn); Hk.SetMat3(6, 15, odMkappa);			// 里程计作用
...
long CFileRdWt::recordsize() 
{
	long records = 0;
	if(columns < 0) {
		records = filesize(1) / (-columns*sizeof(double));
	}
	return records;
}
...
{
	iframe = (iframe >= recordNum) ? recordNum-1 : iframe;
...
CQuat CAligni0::Update(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts, const CVect3 &vel)
{
	...
	Pib02 = Pib02 + vib0*nts, Pi02 = Pi02 + vi0*nts;		// 速度变化量的积分
	//
	if(++t2>3*t0)
	{
	...
	{
		qnb0 = qnb = qnbsb;				// 参考严工敏 捷联惯导7.1.2解析粗对准方法
	}
	else
	{
...
void CAligntf::SetHk(int nnq)
{
	...
	Hk.SetMat3(3,6,-sins.Cnb), Hk.SetClmVect3(3,18,-sins.Cnb*sins.wib);	// Zdatt（姿态不受杆臂影响）
}
...
static int addimudata(imu_t *imu, const imud_t *data)
{
	...
	if (imu->nmax <= imu->n) {
		if (imu->nmax <= 0) imu->nmax = NINCIMU; else imu->nmax += 30000;                  
...
int readimu(imuopt_t *opt, imu_t *imu)
{
	FILE *fp;
	imud_t imud;
	CVect3 wm, vm;
	...
	switch (opt->gyr_type) {
		case 1: wfact *= 1.0/opt->freq;  break;		
		case 2: wfact *= 1.0;   		 break;
		default: return 0;
	}	
	switch (opt->acc_type) {
		case 1: vfact *= 1.0/opt->freq;  break;
		case 2: vfact *= 1.0;   		 break;
		default: return 0;
	}
	switch (opt->gyr_unit) {
		case 1: wfact *= (PI/180.0);     break;
		case 2: wfact *= 1.0;			 break;
		default: return 0;
	}
	switch (opt->acc_unit) {
		case 1: vfact *= 1.0;     		 break;
		case 2: vfact *= glv.g0;		 break;
		default: return 0;
	}
	...
	while (fgets(buff, sizeof(buff), fp)) {
		if (sscanf(buff,"%lf%lf%lf%lf%lf%lf%lf\n", &tow, &wm.i, &wm.j, &wm.k, 
			&vm.i, &vm.j, &vm.k) < 7) continue;
		if (tow < opt->ts || tow > opt->te) continue;
		imud.t = tow;
		imud.wm = wm*wfact;
		imud.vm = vm*vfact;
		IMURFU(&imud.wm, &imud.vm, 1, opt->rfu);
		if (addimudata(imu, &imud) < 0) break;
	}
	fclose(fp);
	return (imu->n>0) ? 1 : 0;
}
