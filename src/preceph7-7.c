	...
	double *bfact, char *tsys)
{
	...
	for (i = 0; i<22; i++) {
		...
		if (i == 0) {
			*type = buff[2];								   //类型
			if (str2time(buff, 3, 28, time)) return 0;        //时间
		}
		else if (2 <= i&&i <= 6) {
			if (i == 2) {
			...
			for (j = 0; j<17 && k<ns; j++) {
				...
				if (k<MAXSAT) sats[k++] = satno(sys, prn);    //卫星号
			}
		}
		else if (i == 12) {
	...
	char *tsys, int index, int opt, nav_t *nav)
{
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		for (i = pred_o = pred_c = v = 0; i<n&&fgets(buff, sizeof(buff), fp); i++) {
			...
			sys = buff[1] == ' ' ? SYS_GPS : code2sys(buff[1]);                 //系统
			...
			for (j = 0; j<4; j++) {
				...
				std = str2num(buff, 61 + j * 3, j<3 ? 2 : 3);    //后面四个：位置、钟差std
	...
	double *dts, double *vare, double *varc)
{
	...
	for (j = 0; j <= NMAX; j++) {
		...
		p[1][j] = pos[1];
#else
		/* correciton for earh rotation ver.2.4.0 */  //地球转动改正
		sinl = sin(OMGE*t[j]);
		cosl = cos(OMGE*t[j]);
	...
	double *rs, double *dts, double *var)
{
	...
	for (i = 0; i<3; i++) {
		rs[i] = rss[i] + dant[i];
		rs[i + 3] = (rst[i] - rss[i]) / tt;   //卫星速度
	}
	/* relativistic effect correction */
	if (dtss[0] != 0.0) {
		dts[0] = dtss[0] - 2.0*dot(rs, rs + 3, 3) / CLIGHT / CLIGHT;
		dts[1] = (dtst[0] - dtss[0]) / tt;   //卫星钟飘
	}
