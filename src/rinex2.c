...
typedef struct {                        /* signal index type */
	...
	int idx[MAXOBSTYPE];                /* signal frequency (1:L1,2:L2,...) */
...
static void decode_obsh(FILE *fp, char *buff, double ver, int *tsys,
	char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta, char *phaseshift)
{
	...
	const char *p;
	char *label = buff + 60, str[4], tmp[20], sys, frq, type;
	...
	else if (strstr(label, "REC # / TYPE / VERS")) {    //���ջ�
		if (sta) {
	...
	else if (strstr(label, "ANTENNA: DELTA H/E/N")) {   //����ƫ��
		if (sta) {
		...
		*tobs[0][nt] = '\0';
		...
		if (phaseshift) {
			if (strstr(buff + 2, "L"))
			{
				...
				frq = buff[3];
				type = buff[4];
				value = str2num(buff, 5, 8);
				sprintf(tmp, "-%cL%c%c=%8.5f ", sys, frq, type, value);
			}
			if (strstr(buff + 2, "L") && value)
				strcpy(phaseshift + strlen(phaseshift), tmp);
		}	
	}
	...
	char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta, char *phaseshift)
{
	...
	*ver = 2.10; *type = ' '; *sys = SYS_GPS;
	...
	while (fgets(buff, MAXRNXLEN, fp)) {
		...
		/* file type */
		switch (*type) {
		case 'O': decode_obsh(fp, buff, *ver, tsys, tobs, nav, sta, phaseshift); break;
	...
	if (ver <= 2.99) { /* ver.2 */
		...
		*flag = (int)str2num(buff, 28, 1);
		...
		for (i = 0, j = 32; i<n; i++, j += 3) {
			if (j >= 68) {
				if (!fgets(buff, MAXRNXLEN, fp)) break;       //���ǻ���
				j = 32;
			}
			if (i<MAXOBS) {
				strncpy(satid, buff + j, 3);
				sats[i] = satid2no(satid);					  //���Ǻţ����֣�
			}
		}
	}
	else { /* ver.3 */
	...
	sigind_t *index, obsd_t *obs)
{
	...
	if (ver>2.99) { /* ver.3 */
		...
		obs->sat = (unsigned char)satid2no(satid);				//ϵͳPRN��ת��Ϊ������ʽ
	}
	if (!obs->sat) {
	...
	/* save obs data */
	for (i = 0; i<ind->n; i++) {
		if (p[i]<0 || val[i] == 0.0 || p[i]>NFREQ + NEXOBS - 1) continue;
		switch (ind->type[i]) {
...
static void saveslips(unsigned char slips[][NFREQ], obsd_t *data)
{
	int i;
	for (i = 0; i<NFREQ; i++) {
		if (data->LLI[i] & 1) slips[data->sat - 1][i] |= LLI_SLIP;     //����|| Ƶ��
	}
}
/* restore slips -------------------------------------------------------------*/
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
	...
	if (obs->nmax <= obs->n) {
		if (obs->nmax <= 0) obs->nmax = NINCOBS; else obs->nmax *= 2;                   //��չ���ݷ�Χ
		if (!(obs_data = (obsd_t *)realloc(obs->data, sizeof(obsd_t)*obs->nmax))) {		//��һ�����ٿռ�
...
static void set_index(double ver, int sys, const char *opt,
	char tobs[MAXOBSTYPE][4], sigind_t *ind, char *phaseshift)
{
	const char *p;
	char str[8], *optstr = "";
	double shift;
	...
	for (i = n = 0; *tobs[i]; i++, n++) {																                              
		ind->code[i] = obs2code(tobs[i] + 1);											//ȷ����ֵ����
		ind->type[i] = (p = strchr(obscodes, tobs[i][0])) ? (int)(p - obscodes) : 0;	//�롢��λ������ȡ�������
	...
	for (p = phaseshift; p && (p = strchr(p, '-')); p++) {							
		if (sscanf(p, optstr, str, &shift)<2) continue;
		for (i = 0; i<n; i++) {
			if (strcmp(code2obs(ind->code[i]), str)) continue;				//code2obs������ת�ַ�
			if (ind->type[i] != 1) continue;										//4.13�޸�
			ind->shift[i] = shift;
	...
	for (i = 0; i<NFREQ; i++) {														//α�� �ز� Ӧ�ð��ղ�ͬ�Ƕ�չ������
		//for (j = 0, k = -1; j<n; j++) {
		...
		//for (j = 0; j<n; j++) {
		//	if (ind->code[j] == ind->code[k]) ind->pos[j] = i;						//code:ͨ��	
		//}
		k0 = k1 = k2 = k3 = -1;														
		for (j = 0; j<n; j++) {
			if (ind->type[j] == 0) {
				if (ind->idx[j] == i && ind->pri[j] && (k0<0 || ind->pri[j]>ind->pri[k0])) {
					k0 = j;
				}
			}
			else if (ind->type[j] == 1) {
				if (ind->idx[j] == i && ind->pri[j] && (k1<0 || ind->pri[j]>ind->pri[k1])) {
					k1 = j;
				}
			}
			else if (ind->type[j] == 2) {
				if (ind->idx[j] == i && ind->pri[j] && (k2<0 || ind->pri[j]>ind->pri[k2])) {
					k2 = j;
				}
			}
			else if (ind->type[j] == 3) {
				if (ind->idx[j] == i && ind->pri[j] && (k3<0 || ind->pri[j]>ind->pri[k3])) {
					k3 = j;
				}
			}
		}
	...
	for (i = 0; i<n; i++) {
		if (!ind->code[i] || !ind->pri[i] || ind->pos[i] >= 0) continue;			//�ޣ�ͨ�������ȼ��� Ƶ��=-1
...
#if 0 /* for debug */
	for (i = 0; i<n; i++) {
		trace(1, "set_index: sys=%2d,tobs=%s code=%2d pri=%2d idx=%d pos=%d shift=%5.2f type=%2d\n",
			sys, tobs[i], ind->code[i], ind->pri[i], ind->idx[i], ind->pos[i],
			ind->shift[i], ind->type[i]);
	}
#endif
}
	...
	sta_t *sta, char *phaseshift)
{
	...
	/* set signal index */
	set_index(ver, SYS_GPS, opt, tobs[0], index, phaseshift);						//index��ϵͳ��ȡ
	set_index(ver, SYS_GLO, opt, tobs[1], index + 1, phaseshift);
	set_index(ver, SYS_GAL, opt, tobs[2], index + 2, phaseshift);
	set_index(ver, SYS_QZS, opt, tobs[3], index + 3, phaseshift);
	set_index(ver, SYS_SBS, opt, tobs[4], index + 4, phaseshift);
	set_index(ver, SYS_CMP, opt, tobs[5], index + 5, phaseshift);					
	set_index(ver, SYS_IRN, opt, tobs[6], index + 6, phaseshift);
	...
	while (fgets(buff, MAXRNXLEN, fp)) {
		...
		if (i == 0) {
			...
			data[n].time = time;																//Rinex2.x  ʱ��|����
			data[n].sat = (unsigned char)sats[i - 1];

			/* decode obs data */
			if (decode_obsdata(fp, buff, ver, mask, index, data + n) && n<MAXOBS) n++;			//opt��ѡ����
		}
		else if (*flag == 3 || *flag == 4) { /* new site or header info follows */

			/* decode obs header */
			decode_obsh(fp, buff, ver, tsys, tobs, NULL, sta, NULL);
		}
		if (++i>nsat) return n;
	}
	return -1;
...
static int readrnxobs(FILE *fp, gtime_t ts, gtime_t te, double tint,
	...
	char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta, char *phaseshift)
{
	obsd_t *data;
	uint8_t slips[MAXSAT][NFREQ] = { { 0 } };
	...
	if (!(data = (obsd_t *)malloc(sizeof(obsd_t)*MAXOBS))) return 0;					//��ǿ������ת����malloc���ֽ�����

	/* read rinex obs data body */
	while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta, phaseshift)) >= 0 && stat >= 0) {

		for (i = 0; i<n; i++) {

			/* utc -> gpst */
			if (*tsys == TSYS_UTC) data[i].time = utc2gpst(data[i].time);				// ϵͳUTCʱ  
...
static int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav)
{
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		//strncmp(str1,str2.n)���Ƚ��ַ���ǰn���ַ�(�ַ���ȷ���0)
		/* only read AS (satellite clock) record */
		if (!strncmp(buff, "AS", 2) && (sat = satid2no(satid)) && (satsys(sat, NULL)&mask)) {
			...
			if (nav->nc <= 0 || fabs(timediff(time, nav->pclk[nav->nc - 1].time))>1E-9) {		//ע���ǣ�-9����������̫������
				nav->nc++;
				nav->pclk[nav->nc - 1].time = time;
				nav->pclk[nav->nc - 1].index = index;						//���ջ���
				for (i = 0; i<MAXSAT; i++) {
			...
			nav->pclk[nav->nc - 1].std[sat - 1][0] = (float)data[1];		//�Ӳ��׼��	
		}
		//��ȡwlbias����
		if (!strncmp(buff, "WL", 2) && (sat = satid2no(satid)) && (satsys(sat, NULL)&mask)) {
			data[0] = str2num(buff, 40, 13);
			nav->wlbias[sat - 1] = data[0];
		}
	}
	return nav->nc>0;
}
	...
	obs_t *obs, nav_t *nav, sta_t *sta)
{
	...
	char tobs[NUMSYS][MAXOBSTYPE][4] = { { "" } }, phaseshift[1024] = "";							//�۲�ֵ�����б�

	trace(3, "readrnxfp: flag=%d index=%d\n", flag, index);

	/* read rinex header */
	if (!readrnxh(fp, &ver, type, &sys, &tsys, tobs, nav, sta, phaseshift)) return 0;
	...
	/* read rinex body */
	switch (*type) {
	case 'O': return readrnxobs(fp, ts, te, tint, opt, index, ver, &tsys, tobs, obs, sta, phaseshift);
	...
	obs_t *obs, nav_t *nav, sta_t *sta)
{
	...
	/* read rinex file */
	stat = readrnxfp(fp, ts, te, tint, opt, flag, index, type, obs, nav, sta);   //����readrnxfp��ȡ�Ӳ� || ��ȡ�۲�ֵ�������ļ�
	fclose(fp);
...
extern int readrnxc(const char *file, nav_t *nav)
{
	...
	/* read rinex clock files */
	for (i = 0; i<n; i++) {
		if (readrnxfile(files[i], t, t, 0.0, "", 1, index++, &type, NULL, nav, NULL)) {				//flag=1��ֻ���Ӳ�
			continue;
		}
		stat = 0;
		break;
...
extern int open_rnxctr(rnxctr_t *rnx, FILE *fp)
{
	...
	if (!readrnxh(fp, &ver, &type, &sys, &tsys, tobs, &rnx->nav, &rnx->sta, NULL)) {
