...
typedef struct {        /* observation data record */
	...
	uint8_t LLI[NFREQ + NEXOBS]; /* loss of lock indicator */    //ʧ��ָʾ��
...
typedef struct {        /* satellite fcb data type */
    ...
    double bias[MAXSAT][3]; /* fcb value   (cyc) [3]->[0]*/
    double std [MAXSAT][3]; /* fcb std-dev (cyc) [3]->[0]*/
	int flag[MAXSAT];
} fcbd_t;
...
typedef struct {        /* solution type */
    ...
    float thres;        /* AR ratio threshold for valiation */    //����ʱ��ratioԼ����ֵ
} sol_t;
...
typedef struct {        /* option type */
    ...
    void *var;          /* pointer to option variable */    //������ָ�����ָ���κ�ָ��
...
typedef struct {        /* processing options type */
    ...
    int dynamics;       /* dynamics model (0:none,1:velociy,2:accel) */			//����ģ�͡��ȼ���ģ��
    int tidecorr;       /* earth tide correction (0:off,1:solid,2:solid+otl+pole) */
    int niter;          /* number of filter iteration */
    int codesmooth;     /* code smoothing window size (0:none) */
    int intpref;        /* interpolate reference obs (for post mission) */     //�����׼�۲�ֵ
    ...
    double thresslipgf; /* slip threshold of geometry-free phase (m) */
	double thresslipmw; /* slip threshold of MW phase (cycle) */
	...
	double interval;	/* process time interval */
} prcopt_t;
...
typedef struct {        /* file options type */     
	...
	char stec   [MAXSTRPATH]; /* stec file */
	char trop   [MAXSTRPATH]; /* trop file */
	char staname[MAXSTRPATH]; /* station name file */
	char indir	[MAXSTRPATH]; /* input obs directory */
	char outdir [MAXSTRPATH]; /* output result directory */
	char snx	[MAXSTRPATH]; /* snx file*/
	char prosta [MAXSTRPATH];  /* process station list*/
	char fcbpos [MAXSTRPATH];
	char outposs[MAXSTRPATH];
	char outtrop[MAXSTRPATH];
	char outiono[MAXSTRPATH];
	char outfcbs[MAXSTRPATH];
	char outelse[MAXSTRPATH];
} filopt_t;
...
typedef struct {        /* satellite status type */
    ...
    unsigned int lock [NFREQ];  /* lock counter of phase����λ�� */
	uint32_t outc[NFREQ];  /* obs outage counter of phase����λ����δʹ��ʱ��+δ�˲�ʱ���� */
	uint32_t slipc[NFREQ];  /* cycle-slip cout(��[0]��������) */
 //   unsigned int rejc [2][NFREQ];  /* reject counter(carrier-phase pseudorange) */
	//unsigned int co   [2][NFREQ];  /* cumulative number of observations */

	double  azel[2];     /* azimuth/elevation angles {az,el} (rad) ��λ�� �߶Ƚ� */
	double  resp[NFREQ]; /* residuals of pseudorange (m) */
	double  resc[NFREQ]; /* residuals of carrier-phase (m) */
	double  icbias[NFREQ];  /* glonass IC bias (cycles) */
	double  LC[7];		  /* linear combination average(m) 0:GF12 1:GF23 2:MW-L12P12 3:MW-L23P23 4:MW-L23P12 5:GF-PL 6:MP*/
    double  phw;        /* phase windup (cycle) */
	double  php;		/* phase prealign (m) */
    gtime_t pt[2][NFREQ]; /* previous pseudorange & carrier-phase time */				
    double  ph[2][NFREQ]; /* previous pseudorange & carrier-phase observable (m & cycle) */
} ssat_t;

typedef struct {        /* ambiguity control type */
	gtime_t epoch[6];
    int n[6];           /* number of epochs */
    double LC [6];      /* linear combination average(m) 0:GF12 1:GF23 2:MW-L12P12 3:MW-L23P23 4:MW-L23P12 5:GF-PL*/			
    double LCv[6];      /* linear combination variance */
} ambc_t;
...
EXPORT int  testsnr(int base, int idx, double el, double snr,
...
EXPORT int  file2nav(const prcopt_t *prcopt, const filopt_t *fopt, nav_t *nav);
...
EXPORT int ionocorr(gtime_t time, const double *pos, const double *azel, const prcopt_t *opt, int sat, const double *x,
...
EXPORT int  model_phw(gtime_t time, int sat, const char *type, int opt, const double *rs, const double *rr, double *phw);
...
EXPORT int  readfcb_sgg(const char *file, nav_t *nav);			//��ȡSGG-FCB
...
EXPORT int lambda_search(int n, int m, const double *a, const double *Q,
                         double *F, double *s);
EXPORT int lambda_PAR(rtk_t *rtk, int na, int nb, int m, const double *a, double *b,
...
EXPORT int  relpos(rtk_t *rtk, const obsd_t *obs, int nu, int nr, const nav_t *nav);
EXPORT void pppinit(rtk_t *rtk, const prcopt_t *opt);
EXPORT void pppfree(rtk_t *rtk);
EXPORT void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav);
EXPORT int	pppnx(const prcopt_t *opt);

/* precise point positioning -------------------------------------------------*/
EXPORT int	ppprocess(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav);
...
EXPORT int  file2corr(const filopt_t *fopt);
EXPORT int  pppcorr_stat(const char *staname, double *pos);
EXPORT int	pppcorr_trop(gtime_t time, const double *pos, double *ztd, double *std);
EXPORT int	pppcorr_stec(gtime_t time, const double *pos, const int sat1, 
	const int sat2, double *ion, double *std);
EXPORT void pppcorr_free();

/* fcb estimate function -----------------------------------------------------*/
EXPORT int fcbpos(gtime_t ts, gtime_t te, const filopt_t *fopt);

/* iono estimate function */
EXPORT int ionoModel(const filopt_t *fopt);

/* post-processing positioning -----------------------------------------------*/
EXPORT int postpos(const prcopt_t *popt, const solopt_t *sopt, filopt_t *fopt);
...
EXPORT int  loadobs(gtime_t ts, gtime_t te,filopt_t *fopt);
...
/* corr meas functions -------------------------------------------------------*/
EXPORT void setpcv(gtime_t time, prcopt_t *popt, nav_t *nav, const pcvs_t *pcvs, const pcvs_t *pcvr, const sta_t *sta);
EXPORT void corr_meas(const obsd_t *obs, const nav_t *nav, const double *azel,
	...
	const double php, double *L, double *P, double *Lc, double *Pc);
...
/* measurement error variance ------------------------------------------------*/
EXPORT void outppp(const char *outfile, const prcopt_t *popt, const char *staname, const double *pos, char type);
EXPORT void outiono(const FILE *fp, const rtk_t *rtk, const obsd_t *obs, const int *n, const nav_t *nav);
EXPORT void outtrop(const FILE *fp, const rtk_t *rtk, const obsd_t *obs, const int *n, const nav_t *nav);
EXPORT void outfcbs(const FILE *fp, const rtk_t *rtk, const obsd_t *obs, const int *n, const nav_t *nav);
EXPORT void outelse(const FILE *fp, const rtk_t *rtk, const obsd_t *obs, const int *n, const nav_t *nav);

/* trop function ------------------------------------------------------------ */
EXPORT void nmf(gtime_t time, const double pos[], const double azel[],
	double *mapfh, double *mapfw);
EXPORT void gmf(double *mjd, double *lat, double *lon, double *hgt, double *zd,
	double *gmfh, double *gmfw);

EXPORT int gpt(double *mjd, double *lat, double *lon, double *hgt, double *pres,
