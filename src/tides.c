                       ...
                       double *dr)
{
    ...
    trace(4,"tide_solid: dr=%.3f %.3f %.3f\n",dr[0],dr[1],dr[2]);
}
#endif /* !IERS_MODEL */
...
static void tide_oload(gtime_t tut, const double *odisp, double *denu)
{
    ...
    ep[3]=ep[4]=ep[5]=0.0;
    days=timediff(epoch2time(ep),epoch2time(ep1975))/86400.0;    //timediff(epoch2time(ep),epoch2time(ep1975))/86400.0+1.0   ÓÐ1cmµÄÓ°Ïì
...
{
    gtime_t tut;
    double pos[3],E[9],drt[3],denu[3],rs[3],rm[3],gmst,erpv[5]={0};
    int i;
#ifdef IERS_MODEL
    double ep[6],fhr;
    int year,mon,day;
