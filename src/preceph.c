	...
	double *bfact, char *tsys)
{
	...
	for (i = 0; i<22; i++) {
		...
		if (i == 0) {
			*type = buff[2];								   //����
			if (str2time(buff, 3, 28, time)) return 0;        //ʱ��
		}
		else if (2 <= i&&i <= 6) {
			if (i == 2) {
				ns = (int)str2num(buff, 3, 3);				   //��4��2����3��3��
			}
			for (j = 0; j<17 && k<ns; j++) {
				sys = code2sys(buff[9 + 3 * j]);
				prn = (int)str2num(buff, 10 + 3 * j, 2);
				if (k<MAXSAT) sats[k++] = satno(sys, prn);    //���Ǻ�
			}
		}
		else if (i == 12) {
			strncpy(tsys, buff + 9, 3); tsys[3] = '\0';		   //ʱ��ϵͳ
		}
		else if (i == 14) {
			bfact[0] = str2num(buff, 3, 10);				   //ϵ��
	...
	char *tsys, int index, int opt, nav_t *nav)
{
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		for (i = pred_o = pred_c = v = 0; i<n&&fgets(buff, sizeof(buff), fp); i++) {
			...
			sys = buff[1] == ' ' ? SYS_GPS : code2sys(buff[1]);                 //ϵͳ
			prn = (int)str2num(buff, 2, 2);                                 //prn
			...
			for (j = 0; j<4; j++) {
				...
				std = str2num(buff, 61 + j * 3, j<3 ? 2 : 3);    //�����ĸ���λ�á��Ӳ�std
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
	...
	/* combine precise ephemeris */
	if (nav->ne>0) combpeph(nav, opt);		// 这个可以加载一下
}
...
static int readdcbf(const char *file, nav_t *nav)
{
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		if ((sat = satid2no(str1))) {						        /* satellite dcb */
...
/* read dcb parameters file --------------------------------------------------*/
static int readdcbssr(const char *file, nav_t *nav)
{
	FILE *fp;
	double cbias;
	char buff[256], obs[4] = "";
	...
	while (fgets(buff, sizeof(buff), fp)) {
		if (!strncmp(buff, "+BIAS/SOLUTION", 14)) {
			while (fgets(buff, sizeof(buff), fp)) {
				if (!strncmp(buff, " OSB", 4) && (sat = satid2no(buff + 11))) {
					strncpy(obs, buff + 25, 3);
					if (obs[0] == 'C' && (code = obs2code(obs + 1) != 0)) {
						if ((cbias = str2num(buff, 84, 7)) == 0.0) continue;
						nav->ssr[sat - 1].cbias[code - 1] = cbias*1E-9*CLIGHT;		/* ns -> m */
					}
					else if (obs[0] == 'L' && (code = obs2code(obs + 1) != 0)) {
						if ((cbias = str2num(buff, 84, 7)) == 0.0) continue;
						nav->ssr[sat - 1].pbias[code - 1] = cbias*1E-9*CLIGHT;		/* ns -> m */
					}
				}
	...
	double *dts, double *vare, double *varc)
{
	...
	for (j = 0; j <= NMAX; j++) {
		...
		p[1][j] = pos[1];
#else
		/* correciton for earh rotation ver.2.4.0 */  //����ת������
		sinl = sin(OMGE*t[j]);
		cosl = cos(OMGE*t[j]);
