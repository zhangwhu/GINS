...
static const double BM = 80.33*D2R;				//  79.91				//2008��شŲ�������������
	...
	*syslist = 0;
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		*syslist |= sys;
		...
		ipp[2] = ionppp(pos, azel, Re, hion, ipp);										//ipp[2]:ͶӰ����   pos:γ�ȡ�����
...
static int generateData(const char *infile)
{
	char stafile[256];
	unsigned char sflag[MAXSAT] = { 0 }, syslist;			
	if (!readstas(infile, &stas)) return 0;

	trace(1,"Generate data....\n");
	for (int i = 0, ircv=0; i < stas.n; i++) {
		checkbrk("(%4d) %s", i, stas.data[i].name);
		sprintf(stafile, "%s%s%s", infile, stas.data[i].name, "-gec-3.ppp");
...
static int outionogrid(const char *outdir, double *lat, double *lon, int nh)
{
	FILE *fp;
	gtime_t ts, te, ti;
	...
	sprintf(outfile, "%s%s", outdir, "iono-g.txt");
...
static int postiono(const char *outdir)   
{
	FILE *fp = NULL;
	gtime_t ts;
	char outfile[256], id[32];
	...
	for (int i = 0; i < tec.n; i++) {
		...
		vtec = (l_tec - gamma*(x[is] + x[ns + ir])) / fact / ipp[2];	/* ��λtecu */
		geo2solarmag(ipp, tt, solar_mag);
		...
		if (ircv_1 != ind2rcv[ir]) {
			sprintf(outfile, "%s%s-g1.iono", outdir, stas.data[rcv2sta[ind2rcv[ir]]].name);
			if (fp == NULL) {
				if (!(fp = fopen(outfile, "w"))) return 0;
			}
			else {
		...
		fprintf(fp, "%s %6s %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f\n", time_str(tec.data[i].time, 0), 
				id, vtec, vtec_cal, fabs(vtec - vtec_cal), l_tec, stec, fabs(l_tec - stec));
	}
	trace(1, "post iono time=%.1f s\n", (tickget() - tick)*0.001);
	fclose(fp);
	return 1;
}
static int drawipp(const char *outdir)
{
	FILE *fp = NULL;
	gtime_t ts;
	char outfile[256];
	...
	sprintf(outfile, "%sipp.txt", outdir);
	if (!(fp = fopen(outfile, "w"))) return 0;
...
extern int ionpos(const char *infile, const char *outdir)	
{			
	double lat[3] = { 37, 57, 0.5 }, lon[3] = { -6, 30, 1.0 };
	//double lat[3] = { 87.5, -87.5, -2.5 }, lon[3] = { -180.0, 180.0, 5.0 };

	/* read station list */
	if (!generateData(infile)) return 0;	  			

	/* process */
	if (!estiono()) return 0;				

	/* output */
	if (!outionogrid(outdir, lat, lon, 1)) return 0;			

	/* post residuals */
	if (!postiono(outdir)) return 0;

	/* draw ipp */
	if (!drawipp(outdir)) return 0;
