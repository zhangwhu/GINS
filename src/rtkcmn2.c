...
const prcopt_t prcopt_default = { /* defaults processing options */
	...
	0.0, 0.0, 0.05, 2, 0.1, 0.01,		/* elmaskar,elmaskhold,thresslipgf,thresslipmw,varholdamb,gainholdamb */
...
static char codepris[7][MAXFREQ][16] = {  /* code priority for each freq-index */
	/*    0         1          2          3         4         5     */
	{ "CPYWMNSL","PYWCMNDLSX","IQX"			,""			,""			,""			,"" }, /* GPS */				// 125	CPYWMNSL--C			PYWCMNDLSX--W		IQX--X
...
extern double code2freq(int sys, uint8_t code, int fcn)
{
	...
	case SYS_GLO: (void)code2freq_GLO(code, fcn, &freq); break;		//DCB�����и����Ļ����Ժ�������ʲô����Ӱ�죬��һ�㱾���ϲ�̫���
...
*-----------------------------------------------------------------------------*/
extern unsigned int getbitu(const unsigned char *buff, int pos, int len)   //����Ĳ���ȷʵ��������ȡ���ݣ��ǳ�������
{
	unsigned int bits = 0;
	int i;
	for (i = pos; i<pos + len; i++) bits = (bits << 1) + ((buff[i / 8] >> (7 - i % 8)) & 1u);  //1u���޷��������� &λ�루10�� >>��7��
	return bits;
}
...
{
	unsigned int mask = 1u << (len - 1);    //ֱ�ӵ�ͷ��Ȼ��ѭ�������
	int i;
	if (len <= 0 || 32<len) return;
	for (i = pos; i<pos + len; i++, mask >>= 1) {  //mask����󲿳���
		if (data&mask) buff[i / 8] |= 1u << (7 - i % 8); else buff[i / 8] &= ~(1u << (7 - i % 8));    //~�������Ʒ���
	}
}
	...
	const pcvs_t *pcvs)
{
	...
	if (sat) { /* search satellite antenna */
		...
		for (i = 0; i<pcvs->n; i++) {
			pcv = pcvs->pcv + i;
			for (j = 0; j < n; j++) {
				if (!strstr(pcv->type, types[j])) {				//��n�ַ���ȫ������	����pcv
					break;
				}
			}
			if (j >= n) {
...
* read positions from station position file(����ļ�ʵ�ʺ��岻��)
...
static int cmpobs(const void *p1, const void *p2)
{
	...
	if (fabs(tt)>DTTOL) return tt<0 ? -1 : 1;						//ʱ��
	if (q1->rcv != q2->rcv) return (int)q1->rcv - (int)q2->rcv;		//��վ
	return (int)q1->sat - (int)q2->sat;							//���Ǻ�
}
...
*-----------------------------------------------------------------------------*/
extern int execcmd(const char *cmd)        //����ϵͳ������˼
{
#ifdef WIN32
...
extern double geodist(const double *rs, const double *rr, double *e)
{
	...
	r = norm(e, 3);             //norm�������
	for (i = 0; i<3; i++) e[i] /= r;
	return r + OMGE*(rs[0] * rr[1] - rs[1] * rr[0]) / CLIGHT;				//�����ǵ����Դ�����
}
...
extern double tropmodel(gtime_t time, const double *pos, const double *azel, int metopt, double humi, int outopt)
{
	...
	if (outopt == 1) trp = trph;								// �ɷ���
	if (outopt == 2) trp = trpw;								// ʪ����
	return trp;
}
	...
	int opt, double *dant)
{
	...
	for (i = 0; i<NFREQ; i++) {
		...
		dant[i] = -dot(off, e, 3) + (opt ? interpvar(90.0 - azel[1] * R2D, azel[0] * R2D, pcv->var[i], pcv->azivar[i], pcv->dazi) : 0.0);
	}
