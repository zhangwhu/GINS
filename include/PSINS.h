...
/************** compiling control !!! ***************/
#define PSINS_MATRIX_MAX_DIM 34				//矩阵最大维度
...
inline BOOL IsZero(double f, double eps = EPS) { return (f < eps && f > -eps); }
...
public:
	...
	friend CVect3 sv2att(const CVect3 &fb, double yaw0, const CVect3 &fn);
...
class CContLarge
{ // data continous to be large value
public:
	double large, dt, t0;
	int cnt, cnti;
	CContLarge(void);
...
class CIMU
{
	CMat3 *pgSens, gSens, *pgSens2, gSens2, *pgSensX, gSensX; // gyro g-sensitivity 物理层面的一些补偿
...
public:
	...
	CVect3 lvr, vnL, posL;
	CMat3 CW, MpvCnb; 		// for lever arm（杆臂处位置信息）
	CQuat qnbE;
	CVect3 attE, vnE, posE; // for extrapolation（外推）
	CMaxMin mmwb, mmfb;
...
public:
	...
	virtual void Feedback(int nnq, double fbts); // state feedback （计算反馈数据值）
...
{
public:
	double meantdts, tdts, Pz0, innovation;	  //一次td的时间间隔
	int iter, ifn, adptOKi, measRes, tdStep, maxStep, curOutStep, maxOutStep;	//tdStep:当前分片滤波一次次数  maxStep:分片滤波一次最大次数
	CMat Fk, Pk1;
	CVect Pxz, Qk, Kk, Hi, tmeas;			  // tmeas: 量测到来时间
	CVect3 meanfn;
	CSINS sins;
	...
	void PSetVertCh(double sph, double spv = 0.0, double spd = 0.0); // vertical channel P set (防止过度收敛)
...
{
public:																// kfts：为了AVPInterp使用
	double posGNSSdelay, vnGNSSdelay, yawGNSSdelay, dtGNSSdelay, dyawGNSS, kfts, *gnsslost; //delay：负号（负-----当前时刻（0）------正）
	int yawHkRow;																			//dt:  （真实）负-------采集时刻（0）------正（真实）
	CVect3 lvGNSS;
	CAVPInterp avpi;			   
...
#ifdef PSINS_IO_FILE
#ifdef PSINS_RMEMORY
class CPOS618 : public CSINSGNSS // POS618 v.s. AV610 事后处理模式
{
	typedef struct
	{
		CVect3 wm, vm;
		double t;
		CVect3 vngnss, posgnss;
		double dt;
	} ImuGnssData;
	typedef struct
	{
...
class CAlignkf : public CSINSGNSS					// Alignkf 也要重构
{
public:
	CVect3 pos0;
	CQuat qnb; 
...
class CAligntrkang : public CSINSGNSS // coarse alignment by GNSS velocity track-angle（动态粗对准）
{
public:
	int cntYawOK;
...
class CAlignsv : public CAlignkf 	 // initial alignment by data save technique 静对准+fine对准
{
public:
...
class CAligntf : public CSINSGNSS // transfer alignment by 'velocity+attitude' method（传递对准）
{
public:
	CVect3 mu, lvMINS; //mu：安装误差角（Cbm）  lvMINS:杆臂
	double dtMINSdelay;
	CAligntf(double ts);
...
class CSINSGNSSOD : public CSINSGNSS // 基础类、不进行处理，方法类
{
public:
...
class CSGOClbt : public CSINSGNSSOD // SIMU/GNSS/OD Calibration（SINS/GNSS dyaw）
{
public:
	CSGOClbt(double ts);
	void Init(const CSINS &sins0, int grade = -1);
...
class CCAM // constant acceleration model 基础类 （单独一个方向）
{
public:
	CVect3 Xk, Qt;			
	double Rpk, Rvk;
...
public:
	...
	int pushLen, popLen, recordLen, recordNum;
	long memLen, dataLen;
...
typedef struct  
{
	...
	int gyr_unit, gyr_type, acc_unit, acc_type;
	int nn, nq, freq, tdStep;         /* imu TDUpdate steps */

	int aln_mode;	     /* Align */
	double aln_t1, aln_t2;
    ...
    CVect3 antlever;    /* lever arm IMU2GNSS */
    CVect3 bodyangle;   /* mouting angles to construct */
...
#include "afx.h"
class CVCFileFind
{ // for file operation in the same folder
public:
	BOOL lastfile;
	char fname[256];
	CFileFind finder;
