                   ...
                   int pcolor, int outalt, int outtime)
{
    ...
    if (tcolor>0) {
        outtrack(fp,solbuf,color[tcolor-1],outalt,outtime);         //轨迹和点
    }
    if (pcolor>0) {
        fprintf(fp,"<Folder>\n");
...
{
    solbuf_t solbuf={0};
    double rr[3]={0},pos[3],dr[3], offset[3] = {0};
    int i,j,nfile,stat;
...
extern int sol2kml(char *path)     
{   
    gtime_t time0 = { 0 };
    int i, n;
    char infile[MAXEXFILE], *files[MAXEXFILE] = { 0 };
    sprintf(infile, "%s*.sol", path);
    ...
    for (i = 0; i<n; i++) {
        convkml(files[i],"", time0, time0, 0.0, 0, 0, 5, 0, 1);
    }
