...
extern void gmf(double *mjd, double *lat, double *lon, double *hgt, double *zzd,
...
extern int gpt(double *mjd, double *lat, double *lon, double *hgt, double *pres,
	double *temp, double *undu)
{
	...
	*temp = temp0 - 0.00650*h_ort;
	...
	free(dfac); free(P); free(Cnm); free(Snm);
}
...
extern void nmf(gtime_t time, const double pos[], const double azel[],
...
extern int pppcorr_trop(gtime_t time, const double *blh, double *trp, double *var, 
	const nav_t *nav)			
{
	int i, k, n, info;
	...
	zwd = mat(1, nav->ntrop);		qq = mat(1, nav->ntrop);		w = mat(1, nav->ntrop);
