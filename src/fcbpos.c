...
#define ROUND(x)    (int)floor((x)+0.5)									//floor(x)���󲻴���x���������
...
static void outarc(arc_t *arcs, char *file)
{
	...
	for (i = 0; i < arcs->n; i++) {
		...
		sat = arcs->data[i].sat;
		satno2id(sat + 1, id);
		if (!(sys = satsys(sat + 1, NULL))) continue;
...
static void outamb(amb_t *ambs, arc_t *arcs, char *file)
{
	...
	for (i = 0; i < ambs->n; i++) {
		...
		satno2id(sat + 1, id);
		if (!(sys = satsys(sat + 1, NULL))) continue;
		fprintf(fp, "%s %s %10.4f %10.4f %4d\n", arcs->name[rcv], id, ambs->data[i].val, ambs->data[i].std, ambs->data[i].len);
	}
	fclose(fp);
}
static void outarcpos(arc_t *arcs, fcb_t *fcb, char *file)
{
	...
	for (i = 0; i < arcs->n; i++) {
		...
		if (fcb->flag[sat] && fcb->flag[IR(k, rcv)]) {
			...
			satno2id(sat + 1, id);
			fprintf(fp, "%s\t", time_str(arcs->data[i].ts, 0));
			fprintf(fp, "%s %s %s %10.4f %10.4f\n", time_str(arcs->data[i].te, 0), arcs->name[rcv], id, FROUND(BN), FROUND(NN));
		}
	}
	fclose(fp);
}
static void outambpos(ambInf *ambs, int n, fcb_t *fcb, arc_t *arcs, char *file)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (fcb->flag[sat] && fcb->flag[IR(k, rcv)]) {
			...
			satno2id(sat + 1, id);
			fprintf(fp, "%4d %s %s %10.4f %10.4f %10.4f\n", ambs[i].ind, arcs->name[rcv], id, FROUND(BN), FROUND(NN), ambs[i].std);
		}
	}
	fclose(fp);
}
...
static double getAverage(double *a, int n, double *std, int ext)
{
	...
	aTmp1 = mat(n, 1);	   aTmp2 = mat(n, 1);		flag = imat(n, 1);
	for (i = 0; i < n; i++) {
		flag[i] = 0;
		aTmp1[i] = FROUND(a[i]);
	...
	if (stdTmp[0] > stdTmp[1]) {
		ave = aveTmp[1];		aTmp = aTmp2;
	}
	else {
		ave = aveTmp[0];		aTmp = aTmp1;
	}
	...
	free(aTmp1);		free(aTmp2);		free(flag);
	return FROUND(ave);
}
static void initarc(arcInf *arc,int rcv,int sat, gtime_t ts, gtime_t te, int lenArc, double val, double std) 
{
	arc->rcv = rcv;		arc->sat = sat;		arc->ts = ts;		arc->te = te;
	arc->val = val;		arc->std = std;		arc->lenArc = lenArc;  	
}
static int addarc(arc_t *arcs, arcInf *data)			
{
	...
	arcInf *arc_data;
	if (arcs->nmax <= arcs->n) {
		if (arcs->nmax <= 0) arcs->nmax = 3000; else arcs->nmax *= 2;
...
{
	free(ambs->data); ambs->data = NULL; ambs->n = ambs->nmax = 0;
}
...
static void arc2amb(arc_t *arcs, amb_t *ambs)		
{
	...
	for (i = 0; i < arcs->nsta; i++) {
		for (j = 0; j < MAXSAT; j++) {
			narc = arcs->narc[i][j];
			if (narc > 0) {
				for (k = 0, std = 1E4; k < narc; k++) {
					if (std > arcs->data[arcs->index[i][j][k]].std) {
						val = arcs->data[arcs->index[i][j][k]].val;
...
static int inifcb(const ambInf *amb, int num, int *flag, double *ini)		
{
	...
	for (i = 0; i < MAXRCV; i++) rNum[i] = 0;
	for (i = 0; i < MAXSAT; i++) { maxSat[i] = 0; ind[i] = 1E4; }
	for (i = 0; i < MAXSAT + MAXRCV; i++) flag[i] = 0;
	for (i = 0; i < MAXRCV; i++) for (j = 0; j < MAXSAT; j++) len[i][j] = 0;
	for (i = 0, temp = 0; i < num; i++) {
		rcv = amb[i].rcv;				sat = amb[i].sat;
		val[rcv][sat] = amb[i].val;		len[rcv][sat] = amb[i].len; 
		std[rcv][sat] = amb[i].std;		maxSat[sat]++;							
		if (temp < maxSat[sat]) {
			temp = maxSat[sat];				
			ref = sat;
		}
	}
	flag[ref] = 1;				ini[ref] = 0;			ind[ref] = nfix;

	while (1) {
		for (sat = 0; sat < MAXSAT; sat++) { sNum[sat] = 0; is[sat] = sat; }
		for (rcv = 0; rcv < MAXRCV; rcv++) {
			for (sat = 0, temp = 1E2, ref = -1; sat < MAXSAT; sat++) {						
				if (flag[sat] && len[rcv][sat] > 0 && ind[sat] < temp) {	     		
					temp = ind[sat];		ref = sat;
				}
			}
			if (ref < 0) continue;
		...
		for (i = 0; i < MAXSAT - 1; i++) for (j = i + 1; j < MAXSAT; j++) {
		...
		for (i = exc = 0; i< MAXSAT; i++) {
			...
			if (sNum[sat] >= NS) {			
				...
				if (1/*std0 < THRESIN*/) {				
					exc = 1;		flag[sat] = 1;			ini[sat] = tempd;		ind[sat] = ++nfix;
	...
	for (rcv = 0; rcv < MAXRCV; rcv++) {
		if (rNum[rcv] >= NR) {
			...
			if (1/*std0 < THRESIN*/) {
				flag[MAXSAT + rcv] = 1;		  ini[MAXSAT + rcv] = tempd;
...
static int lsqfcb(const ambInf *amb, int num, int *flag, double *ini, double *sol)
{
	...
	for (i = 0; i < MAXRCV; i++) for (j = 0; j < MAXSAT; j++) len[i][j] = 0;
	for (i = 0, nsat = 0; i < MAXSAT; i++) if (flag[i]) isat[nsat++] = i;
	for (i = 0, nrcv = 0; i < MAXRCV; i++) if (flag[MAXSAT + i]) ircv[nrcv++] = i;
	...
	nx = nsat + nrcv;			x0 = mat(nx, 1);
	x = zeros(nx, 1);			dx = zeros(nx, 1);		v = zeros(num + 1, 1);		H = zeros(num + 1, nx);		Q = zeros(nx, nx);
	
	for (i = 0; i < nsat; i++) x0[i] = ini[isat[i]];
	for (i = 0; i < nrcv; i++) x0[nsat + i] = ini[MAXSAT + ircv[i]];
	...
	for (i = 0; i < nsat; i++) sol[isat[i]] = x[i];
	for (i = 0; i < nrcv; i++) sol[MAXSAT + ircv[i]] = x[nsat + i];

	free(x0);		free(x);		free(dx);		free(v);		free(H);		free(Q);
	return stat;
}
	...
	for (k = 0; k < 4; k++) {		
		...
		if (n > 0) {
			if (inifcb(&amb0, n, flag, ifcb) && lsqfcb(&amb0, n, flag, ifcb, sfcb)) {				
				for (i = 0; i < MAXSAT; i++) if(flag[i]){
					fcb->flag[IS(i)] = flag[i];
					fcb->ini[IS(i)] = ifcb[i];
					fcb->sol[IS(i)] = sfcb[i];
				}
				for (i = 0; i < MAXRCV; i++) if (flag[MAXSAT+i]) {
...
static int cmpamb(const void *p1, const void *p2)
{
	...
	if (q1->ind != q2->ind) return (int)q1->ind - (int)q2->ind;		//ʱ��
	if (q1->rcv != q2->rcv) return (int)q1->rcv - (int)q2->rcv;		//��վ
	if (q1->sat != q2->sat) return (int)q1->sat - (int)q2->sat;		//���Ǻ�
}
static int sortamb(amb_t *ambs)
{
	if (ambs->n <= 0) return 0;
...
static int outhead(char *outfile, fcb_t *el, fcb_t *wl) 
{
	...
	if (!(fp = fopen(outfile, "w"))) {
		showmsg("error : open output file %s", outfile); return 0;
	}
	/* output header */
	fprintf(fp, "Widelane Satellite Fractional Cycle Biases                  COMMENT\n");
	if (el) {
		for (int i = 0; i < MAXSAT; i++) {
			if (!el->flag[i]) continue;
			satno2id(i + 1, id);
...
static void read_arc(gtime_t ts, gtime_t te, const char *file, arc_t *arcs, int type)
{
	...
	int i, slip, sat, rcv, ns, stat = 0, sflag[MAXSAT] = { 0 }, sflag_1[MAXSAT] = { 0 };
	double azel, EL, Pe, WL, Pw, IF, Pf, NN, std, std1, std2, thread;
	char *p, buff[256] = "", ymd[12] = "", dms[10] = "", satid[8] = "", time[32] = "";

	rcv = arcs->nsta;  ns = arcs->n;
	...
	while (fgets(buff, sizeof(buff), fp)) {
		if ((p = strstr(buff, "staname"))) {
			strncpy(arcs->name[rcv], buff + 14, 4);
			arcs->name[rcv][4] = '\0';
		}
		else if ((p = strstr(buff, "position"))) {
			arcs->rr[rcv][0] = str2num(buff, 13, 14);
			arcs->rr[rcv][1] = str2num(buff, 27, 14);
			arcs->rr[rcv][2] = str2num(buff, 42, 14);
		}
		else if (buff[0] != '%') {
			if (sscanf(buff, "%s %s %s %d %lf %lf %lf %lf %lf %lf %lf", ymd, dms, satid, &slip, &azel, &EL, &Pe, &WL, &Pw, &IF, &Pf) < 11) {
				continue;
			}
			sprintf(time, "%s %s", ymd, dms);
			str2time(time, 0, 32, &ti);
			sat = (unsigned char)satid2no(satid) - 1;
			if (timediff(ts, ti) > 0 || timediff(ti, te)>0) continue;
			switch (type) {
			case 0: NN = WL; std = Pw; thread = THRESWL; break;
			case 1: NN = EL; std = Pe; thread = THRESEL; break;
			default: NN = 0;
			}

			if (timediff(ti, t_1) != 0.0) {
				for (i = 0; i < MAXSAT; i++) {
					if (sflag_1[i] && !sflag[i]) {						// ǰ����Ԫ���ݣ�����Ԫ��ǰ��Ԫ&ǰǰ��Ԫ��
	...
	arcs->nsta++;
	if (arcs->n - ns < 10) { arcs->n = ns; arcs->nsta--; }
}
static void read_amb(gtime_t ts, gtime_t te, const char *file, arc_t *arc_wl, amb_t *amb_nl) 
{
	...
	int i, j, k, rcv, slip, ind, sat, sys, id, stat = 0, num[MAXIND][MAXSAT], exc[MAXIND][MAXSAT];
	double azel, EL, Pe, WL, Pw, IF, Pf, NL, NN, val[MAXIND][MAXSAT], std[MAXIND][MAXSAT], minstd[MAXIND][MAXSAT];
	...
	while (fgets(buff, sizeof(buff), fp)) {
		if ((p = strstr(buff, "staname"))) {
			for (i = 0; i < arc_wl->nsta; i++) {
				if (!strncmp(arc_wl->name[i], buff + 14, 4)) {
					rcv = i;	 stat = 1;		break;
				}
			}
		}
			...
			if (!(sys = satsys(sat + 1, NULL)) || slip || Pf > THRESIF || azel < 15) continue;
			...
			for (k = 0; k < arc_wl->narc[rcv][sat]; k++) {
				...
				if (arc_wl->data[id].flag && (timediff(arc_wl->data[id].ts, ti) < 0) && (timediff(arc_wl->data[id].te, ti) >= 0)) {
					switch (sys) {
					case SYS_GPS: NL = IF*(FREQ1 + FREQ2) / CLIGHT - arc_wl->data[id].NN * FREQ2 / (FREQ1 - FREQ2);	break;
					case SYS_GAL: NL = IF*(FREQ1 + FREQ5) / CLIGHT - arc_wl->data[id].NN * FREQ5 / (FREQ1 - FREQ5);	break;
					case SYS_CMP: NL = IF*(FREQ1_CMP + FREQ3_CMP) / CLIGHT - arc_wl->data[id].NN * FREQ3_CMP / (FREQ1_CMP - FREQ3_CMP);	break;
					default: continue;
					}
	...
	for (i = 0; i < MAXIND; i++) {
		for (j = 0; j < MAXSAT; j++) {
			if (exc[i][j]) {
				...
				if ((stat = addamb(amb_nl, &amb0)) < 0) return;
			}
		}
	}
}
extern int fcbpos(gtime_t ts, gtime_t te, const filopt_t *fopt)					
{
	int i, n;
	ambInf ambs[4000];
	...
	/* output name */
	sprintf(fcbfile, "%s%s", fopt->outdir, "ini-new.fcb");
	sprintf( nlfile, "%s%s", fopt->outdir, "pos-new.txt");
	...
	n = expath(fopt->fcbpos, files, MAXEXFILE);
	...
	if (!outhead(fcbfile, &fcb_el, &fcb_wl)) return -1;
	...
	freearc(&arc_el);	freearc(&arc_wl);
	freeamb(&amb_el);	freeamb(&amb_wl);	freeamb(&amb_nl);
