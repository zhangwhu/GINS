/*------------------------------------------------------------------------------
* ionex.c : ionex functions（格网电离层产品）
*
                         ...
                         double *rb, double *nexp, double *dcb, double *rms)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        else if (strstr(label,"EXPONENT")==label) {          //指数
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"START OF TEC MAP")==label) {
            ...
            for (m=0;m<n;m++) {
                ...
                if (type==1) p->data[index]=x*pow(10.0,nexp);				//pow(10.nexp):将单位转化为TECU
...
static void combtec(nav_t *nav)
{
    ...
    for (i=0;i<nav->nt-1;i++) {
        for (j=i+1;j<nav->nt;j++) {
            if (timediff(nav->tec[j].time,nav->tec[i].time)<0.0) {      //排序
                tmp=nav->tec[i];
                nav->tec[i]=nav->tec[j];
                nav->tec[j]=tmp;
            }
    ...
    for (i=0;i<nav->nt;i++) {
        if (i>0&&timediff(nav->tec[i].time,nav->tec[n-1].time)==0.0) {   //排除数据
...
/* interpolate tec grid data -------------------------------------------------*/   //插值格网数据
                    ...
                    const double *azel, int opt, double *delay, double *var)
{
    ...
    *delay=*var=0.0;
    ...
    for (i=0;i<tec->ndata[2];i++) { /* for a layer */					//一般是单层模型
        ...
        if (!interptec(tec,i,posp,&vtec,&rms)) return 0;				//根据穿刺点计算vtec
